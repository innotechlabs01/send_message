...
CREATE TABLE IF NOT EXISTS mensajes_prediseniados (
  id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_mensajes_categoria ON mensajes_prediseniados(categoria_id) WHERE activo = true;
...
CREATE INDEX IF NOT EXISTS idx_apikeys_hash ON api_keys(key_hash) WHERE activa = true;
...
CREATE INDEX IF NOT EXISTS idx_mp_fecha_estado ON mensajes_programados(fecha_envio, estado) WHERE estado = 'pendiente';
CREATE INDEX IF NOT EXISTS idx_mp_user ON mensajes_programados(user_id);
CREATE INDEX IF NOT EXISTS idx_mp_empresa ON mensajes_programados(empresa_id);
...
DROP TRIGGER IF EXISTS trg_mensajes_programados_updated_at ON mensajes_programados;
...
CREATE INDEX IF NOT EXISTS idx_logs_evento ON audit_logs(evento, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_logs_ip ON audit_logs(ip, created_at DESC);

-- 002: RLS Policies
ALTER TABLE mensajes_programados ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "usuario_propio" ON mensajes_programados;
CREATE POLICY "usuario_propio" ON mensajes_programados
  FOR ALL USING (auth.uid() = user_id);

ALTER TABLE api_keys ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "solo_service_role_apikeys" ON api_keys;
CREATE POLICY "solo_service_role_apikeys" ON api_keys
  FOR ALL USING (auth.role() = 'service_role');

ALTER TABLE audit_logs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "solo_service_role_logs" ON audit_logs;
CREATE POLICY "solo_service_role_logs" ON audit_logs
  FOR ALL USING (auth.role() = 'service_role');

ALTER TABLE categorias ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "lectura_publica_cat" ON categorias;
CREATE POLICY "lectura_publica_cat" ON categorias
  FOR SELECT USING (activa = true);
DROP POLICY IF EXISTS "escritura_service_role_cat" ON categorias;
CREATE POLICY "escritura_service_role_cat" ON categorias
  FOR ALL USING (auth.role() = 'service_role');

ALTER TABLE mensajes_prediseniados ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "lectura_publica_msg" ON mensajes_prediseniados;
CREATE POLICY "lectura_publica_msg" ON mensajes_prediseniados
  FOR SELECT USING (activo = true);
DROP POLICY IF EXISTS "escritura_service_role_msg" ON mensajes_prediseniados;
CREATE POLICY "escritura_service_role_msg" ON mensajes_prediseniados
  FOR ALL USING (auth.role() = 'service_role');

-- 005: Configuración de pago
...
DROP TRIGGER IF EXISTS trg_configuracion_pago_updated_at ON configuracion_pago;
...
DROP POLICY IF EXISTS "lectura_publica_config" ON configuracion_pago;
CREATE POLICY "lectura_publica_config" ON configuracion_pago
  FOR SELECT USING (activa = true);
DROP POLICY IF EXISTS "escritura_service_role_config" ON configuracion_pago;
CREATE POLICY "escritura_service_role_config" ON configuracion_pago
  FOR ALL USING (auth.role() = 'service_role');

INSERT INTO configuracion_pago (precio_cop) VALUES (2380);

-- 006: Campos de contacto
...
-- 008: Políticas públicas
DROP POLICY IF EXISTS "public_insert_sin_usuario" ON mensajes_programados;
CREATE POLICY "public_insert_sin_usuario" ON mensajes_programados
  FOR INSERT WITH CHECK (user_id IS NULL);

DROP POLICY IF EXISTS "public_select_sin_usuario" ON mensajes_programados;
CREATE POLICY "public_select_sin_usuario" ON mensajes_programados
  FOR SELECT USING (user_id IS NULL);

-- 20260430: Tabla de pagos
...
SELECT id, msg FROM categorias, (VALUES
  ...
  ('Tú eres un gran regalo que no necesita envoltura. Que este día sea tan special como tú y estés rodeado de todos tus seres amados. ¡Felicidades!'),
...
SELECT id, msg FROM categorias, (VALUES
  ...
  ('Aunque no esté físicamente, su esence vive en quienes lo amaron.'),
